...
\begin{document}
	...
	\chapter{Introduction}
		...
		\section{Internship context}
			...
			Finally, we'll conclude with some personal observations and difficulties encountered and open some roads for future investigation.
			\par
			\bigbreak
	...
	\chapter{Action recognition in egocentric vision}
		...
		\section{Results and interpretation}\label{results}
			...
			\end{figure}
			\footnotetext{The weird peaks in the curve are due to the training process described in \ref{memory}.}
	...
	\chapter{Conclusion}
		\section{Results synthesis} %TODO
			Lorem ipsum.
		\section{Roads for investigation} %TODO
			Lorem ipsum.
		\section{Acquired knowledge and skills} %TODO
			...
			This project shown me the difficulty to work with deep architectures, especially debugging such huge black boxes.
			After all the literature reviewing on the subject, I learned many things about recent developed techniques and architectures.
	...
	\chapter{Data type}\label{appendix_d}
		...
		%TODO: change legends name in plots if enough time
		\begin{itemize}[noitemsep]
			\item \makebox[3cm][l]{$float32$} compressed $float32$
			\item \makebox[3cm][l]{$float32\_npz$} uncompressed $float32$
			\item \makebox[3cm][l]{$uint8$} compressed $uint8$
			\item \makebox[3cm][l]{$uint8\_npz$} uncompressed $uint8$
		\end{itemize}
		...
		\begin{figure}[h!]
			\centering
			\subfloat[disk size]{\includegraphics[width=0.47\textwidth]{disk_size.png}\label{disk_size}}
			\hfill
			\subfloat[RAM size]{\includegraphics[width=0.47\textwidth]{ram_size.png}\label{ram_size}}
			\vfill
			\subfloat[loading time]{\includegraphics[width=0.5\textwidth]{load_time.png}\label{load_time}}
