...
\begin{document}
	...
	\chapter{Action recognition in egocentric vision}
		...
		\section{State-of-the-art}
			...
			\subsection{Two-stream architectures}\label{twostream}
				...
				\subsubsection{Literature}
					...
					In \cite{ye2019two}, this idea of using the temporal correlation of the images makes use of Recurrent Neural Networks (RNN) through ConvLSTM \cite{shi2015convolutional}.
					...
					However, as we consider images it would be a waste to shrink the images into vectors for feeding them to a LSTM; it will result in a loss of information.
					To overcome this, Shi et al. \cite{shi2015convolutional} developed the ConvLSTM cell, that allows to process images inside a LSTM.
					...
					They incorporate this mechanism inside a ConvLSTM and achieve the best results to this date.
		...
		\section{Studied methods}
			\subsection{Transfer Learning}
				...
				\subsubsection{Feature visualization}
					...
					\par
					Already before, Erhan et al. \cite{erhan2009visualizing} applied this principle of activation maximization on different architectures trained on MNIST.
				...
				Thus we designed our architectures using MobileNet V2 pre-trained on ImageNet as a features extractor.
				...
				Additionally, MobileNet V2 uses residual blocks similar to ResNet, which has been shown to help with gradient vanishing and exploding.
		...
		\section{Results and interpretation}
			All the different architectures that we tried share the same frameworks:
			\begin{itemize}
				\item We draw 20 uniformly spaced RGB images from the action video that we feed.
				\item Each image is fed to the pre-trained MobileNet V2.
				\item The resulting feature maps are then either spacially averaged, for LSTM and temporal convolution, either directly sent to a ConvLSTM.
			\end{itemize}
		\section{Tools}
			Lorem ipsum.
	\chapter{Conclusion}
	...
	\chapter{Reccurent Neural Networks / Long-Short Term Memory}\label{appendix_b}
		...
		\begin{figure}[h!]
			\centering
			\subfloat[RNN cell.]{\includegraphics[width=0.55\textwidth]{description-block-rnn-ltr.png}\label{rnn_cell}}
			\hfill
		...
		Note: the flow of information through the cell state, top line of \ref{lstm_cell}, has been shown to help counter gradient vanishing.
