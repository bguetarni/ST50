		...
		\section{Studied methods}
			...
			\subsection{Architectures base}
				...
				\begin{enumerate}
					...
					\item The error backpropagation is applied to the temporal processing subsystem only.
				\end{enumerate}
				\begin{figure}[h!]
					\centering
					\subfloat{\includegraphics[width=1.2\textwidth]{architecture_diagr_1.png}\label{common_base}}
					\caption{Common base.}
				\end{figure}
				We use the categorical cross-entropy error between predicted and ground-truth labels.
				\begin{align*}
					L^{verb} = -\sum_{i} y^{verb}_{i}log(\hat{y}^{verb}_{i}) \\
					L^{noun} = -\sum_{i} y^{noun}_{i}log(\hat{y}^{noun}_{i}) \\
					Loss = L^{verb} + L^{noun} \\
				\end{align*}
				where $y$ and $\hat{y}$ are the ground-truth and prediction respectively.
